...
💡 <strong>الحل:</strong> افتح <strong>لوحة الأدمن ⚙️</strong> (Tab 2) واحصل على مفتاح مجاني جديد من <a href="https://aistudio.google.com/app/apikey" target="_blank" style="color:var(--gold);">aistudio.google.com/app/apikey</a> ثم اضغط <strong>حفظ وتطبيق</strong>.`;
    ...
    // ============================================================
    // CHATBOT (REAL GEMINI + INTELLIGENT FALLBACK)
    // ============================================================
    ...
    const chatSend = document.getElementById('ai-chat-send-btn');
    const pills = document.querySelectorAll('.prompt-pill');

    if (chatTrigger && chatDrawer) {
        chatTrigger.addEventListener('click', () => { chatDrawer.classList.toggle('active'); if (chatInput && chatDrawer.classList.contains('active')) chatInput.focus(); });
        if (chatClose) chatClose.addEventListener('click', () => chatDrawer.classList.remove('active'));

        function appendMsg(html, sender, id = null) {
            const d = document.createElement('div'); d.className = `chat-msg ${sender}-msg`; if (id) d.id = id;
            const av = document.createElement('div'); av.className = 'msg-avatar'; av.innerHTML = sender === 'bot' ? '<i class="fa-solid fa-robot"></i>' : '<i class="fa-solid fa-user"></i>';
            const cn = document.createElement('div'); cn.className = 'msg-content'; cn.innerHTML = html;
            d.appendChild(av); d.appendChild(cn); chatMsgs.appendChild(d);
        }

        // FIX BUG-04: Fallback uses real current prices
        function fallbackResponse(txt) {
            const t = txt.toLowerCase();
            if (t.includes('ذهب') || t.includes('gold') || t.includes('xau')) {
                const p = state.prices.XAUUSD.price, ta = TA.analyze('XAUUSD');
                return `🎯 <strong>الذهب الحي (RSI: ${ta.rsi}):</strong><br>السعر: <strong class="text-gold">$${p.toLocaleString()}</strong><br>الإشارة: <strong class="${ta.signal === 'BUY' ? 'text-success' : 'text-danger'}">${ta.signal === 'BUY' ? 'شراء 🟢' : 'بيع 🔴'}</strong><br>TP1: $${(p + (ta.signal === 'BUY' ? 35 : -35)).toFixed(2)} | SL: $${(p - (ta.signal === 'BUY' ? 25 : -25)).toFixed(2)}`;
            }
            if (t.includes('نفط') || t.includes('oil')) {
                const p = state.prices.USOIL.price, ta = TA.analyze('USOIL');
                return `🛢️ <strong>النفط (WTI) الحي:</strong><br>السعر: <strong class="text-oil">$${p.toFixed(2)}</strong><br>RSI: ${ta.rsi} | ${ta.signal === 'BUY' ? '🟢 صاعد' : '🔴 هابط'}`;
            }
            if (t.includes('بتكوين') || t.includes('btc')) {
                const p = state.prices.BTCUSD.price, ta = TA.analyze('BTCUSD');
                return `₿ <strong>البتكوين الحي:</strong><br>السعر: <strong class="text-crypto">$${p.toLocaleString()}</strong><br>RSI: ${ta.rsi} | ${ta.signal === 'BUY' ? '🟢 شراء' : '🔴 بيع'}`;
            }
            if (t.includes('فيدرالي') || t.includes('fed') || t.includes('تضخم')) {
                const m = Macro.summary();
                return `🏛️ <strong>تقرير الفيدرالي:</strong><br>التوجه: <strong class="text-gold">${m.fedBias}</strong><br>التضخم: ${m.inflation} | NFP: ${m.nfp}<br>مخاطر: ${m.geoRisk}`;
            }
            if (t.includes('استراتيجية') || t.includes('خطة')) {
                return `📊 <strong>استراتيجية Neural AI:</strong><br><ol style="margin-right:1rem;"><li>خاطر بـ <strong>1.5%</strong> فقط من رصيدك.</li><li>نسبة عائد لا تقل عن <strong>1:2.5</strong>.</li><li>انقل SL للدخول بعد TP1.</li></ol>`;
            }
            const gp = state.prices.XAUUSD.price, bp = state.prices.BTCUSD.price;
            return `🤖 <strong>نظرة السوق الآن:</strong><br>🥇 الذهب: <strong class="text-gold">$${gp.toLocaleString()}</strong> — ${TA.analyze('XAUUSD').signal === 'BUY' ? '🟢 شراء' : '🔴 بيع'}<br>₿ BTC: <strong class="text-crypto">$${bp.toLocaleString()}</strong><br><small>💡 أضف Gemini API key في الأدمن لتحليل أعمق!</small>`;
        }

        function formatMarkdownHtml(str) {
            if (!str) return '';
            return str
                .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
                .replace(/### (.*?)\n/g, '<h5 style="color:var(--gold); margin-top:0.5rem; margin-bottom:0.25rem;">$1</h5>')
                .replace(/## (.*?)\n/g, '<h4 style="color:var(--gold); margin-top:0.5rem; margin-bottom:0.25rem;">$1</h4>')
                .replace(/- (.*?)\n/g, '• $1<br>')
                .replace(/\n/g, '<br>');
        }

        async function sendMsg(custom = null) {
            const text = custom || chatInput?.value.trim();
            if (!text) return;
            if (chatInput && !custom) chatInput.value = '';
            appendMsg(text, 'user');
            chatMsgs.scrollTop = chatMsgs.scrollHeight;
            const tid = 'typing-' + Date.now();
            appendMsg('<i class="fa-solid fa-spinner fa-spin"></i> يفكر المستشار المالي...', 'bot', tid);
            chatMsgs.scrollTop = chatMsgs.scrollHeight;
            let reply = null;
            if (state.aiConfig.geminiKey) reply = await GeminiAI.chat(text, null);
            const el = document.getElementById(tid); if (el) el.remove();
            appendMsg(reply ? formatMarkdownHtml(reply) : fallbackResponse(text), 'bot');
            chatMsgs.scrollTop = chatMsgs.scrollHeight;
        }
        ...
        pills.forEach(p => p.addEventListener('click', e => sendMsg(e.currentTarget.textContent.trim())));
    }
